...
import { INVENTORY_KINDS, LINE_CATEGORIES } from "@/lib/kinds";
...
function triagePrompt(item: DigestEntry): { label: string; prompt: string } | null {
  ...
  switch (item.kind) {
    case "low_stock":
      return ref ? { label: "Draft restock", prompt: `Draft a restock plan for ${ref}.` } : null;
    case "slow_mover":
      return ref ? { label: "Ask", prompt: `${ref} is moving slowly. What would you propose?` } : null;
    case "order_issue":
      return {
        label: "Draft reply",
...
function context(item: DigestEntry) {
  if (item.listing) {
    const lines = LINE_CATEGORIES.has(item.listing.category ?? "");
    return (
      <span className="am-mono">
        {item.listing.listing_id} · {formatNumber(item.listing.stock)} {lines ? "lines" : "left"} · {formatMoney(item.listing.price)}
        {lines ? "/mo" : ""}
      </span>
    );
  }
