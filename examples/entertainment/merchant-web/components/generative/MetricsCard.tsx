...
const CURRENCY_METRICS = new Set(["sales", "revenue", "gross", "average_order_value", "average_ticket_price", "spend"]);
const RATE_METRICS = new Set(["conversion_rate", "sell_through", "sell_through_pct", "return_rate", "click_through_rate"]);

function metricLabel(metric: string): string {
  if (metric === "average_order_value") return "Average order";
  if (metric === "tickets") return "Tickets sold";
          ...
          return (
            <div key={`${entry.metric}-${index}`} className="px-3.5 py-3">
              ...
              <div className="mt-1 flex items-baseline gap-2">
                {value != null ? <span className="at-mono text-[19px] font-semibold leading-none tracking-[-0.02em] text-(--ink)">{value}</span> : null}
